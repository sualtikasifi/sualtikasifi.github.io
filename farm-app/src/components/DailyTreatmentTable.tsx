...
// Aktif tedavi kurlerinden bugunun gorev tablosu: hangi buzagiya bugun
// protokolun kacinci gunu ve hangi ilaclar uygulanacak.
export function DailyTreatmentTable({ courses, protocols, protocolDays, animals, treatments, canManage, onLogDone }: Props) {
  ...
  for (const course of courses) {
    ...
    const done = treatments.some((t) => t.course_id === course.id && t.treatment_date === today);
    rows.push({ course, earTag: animal?.ear_tag ?? "?", protocolName: protocol.name, day, maxDay, medicines: meds, done });
  }
  ...
  return (
    <div className="card space-y-2">
      <h2 className="text-sm font-semibold text-neutral-800">Bugünün Tedavi Görevleri</h2>
